...
BASE_MODEL_COLUMNS = ["Logistic Regression", "Random Forest", "XGBoost", "LightGBM"]
...
def session_selected_upload_no(result_frame: pd.DataFrame, default_no: int) -> int:
    selected_no = int(st.session_state.get("upload_selected_no", default_no))
    source = st.session_state.get("upload_selection_source")

    chart_state = st.session_state.get("upload_probability_chart")
    table_state = st.session_state.get("upload_result_table")

    chart_no = None
    try:
        points = chart_state.selection.points
        if points:
            chart_no = int(points[0]["customdata"])
    except Exception:
        pass
    ...
    try:
        ...
        if rows:
            table_no = int(result_frame.iloc[int(rows[0])]["No"])
    except Exception:
        pass

    if source == "chart" and chart_no is not None:
        selected_no = chart_no
    elif source == "table" and table_no is not None:
        selected_no = table_no
    elif table_no is not None:
        selected_no = table_no
    elif chart_no is not None:
        selected_no = chart_no

    return selected_no


def batch_probability_chart(result_frame: pd.DataFrame, selected_no: int | None = None):
    if result_frame.empty:
        return go.Figure()

    frame = result_frame.sort_values("연체 확률(%)", ascending=True).copy()
    labels = frame["No"].astype(str) + " · " + frame["고객명"].astype(str)
    ...
    if selected_no is not None:
        selected_points = [
            idx for idx, no_value in enumerate(frame["No"].astype(int).tolist())
            if no_value == int(selected_no)
        ]
        colors = [
            "#8d5963" if int(no_value) == int(selected_no) else color
            for no_value, color in zip(frame["No"].astype(int).tolist(), colors)
        ]
    ...
    fig = go.Figure(
        go.Bar(
            ...
            selected={"marker": {"color": "#8d5963", "opacity": 1}},
...
def selected_upload_no(chart_state, table_state, table_frame: pd.DataFrame, default_no: int) -> int:
    selected_no = int(st.session_state.get("upload_selected_no", default_no))
    ...
    try:
        ...
        points = []
    if points:
        selected_no = int(points[0]["customdata"])
    ...
    source = st.session_state.get("upload_selection_source")
    if source == "table" and rows:
        selected_no = int(table_frame.iloc[int(rows[0])]["No"])
    elif source == "chart" and points:
        selected_no = int(points[0]["customdata"])
    elif rows:
        selected_no = int(table_frame.iloc[int(rows[0])]["No"])
    elif points:
    ...
    st.session_state["upload_selected_no"] = selected_no
    return selected_no


def base_probabilities_from_result(row: pd.Series) -> dict[str, float]:
    return {column: float(row[column]) / 100 for column in BASE_MODEL_COLUMNS if column in row.index}


def mark_upload_chart_selection():
    st.session_state["upload_selection_source"] = "chart"


def mark_upload_table_selection():
    st.session_state["upload_selection_source"] = "table"
    ...
    if uploaded_csv is not None:
        ...
        if not upload_frame.empty:
            ...
            if missing_columns:
                ...
                result_frame, _ = make_upload_predictions(artifact, upload_frame)
                ...
                default_no = int(result_frame.iloc[0]["No"])
                preselected_no = session_selected_upload_no(result_frame, default_no)

                st.markdown('<div class="probability-panel"><div class="panel-head">인보이스별 연체 확률</div></div>', unsafe_allow_html=True)
                chart_state = st.plotly_chart(
                    batch_probability_chart(result_frame, preselected_no),
                    width="stretch",
                    key="upload_probability_chart",
                    on_select=mark_upload_chart_selection,
                ...
                table_frame = result_frame[display_columns].copy()
                table_state = st.dataframe(
                    table_frame,
                    hide_index=True,
                    width="stretch",
                    height=min(520, 76 + len(table_frame) * 36),
                    key="upload_result_table",
                    on_select=mark_upload_table_selection,
                    selection_mode="single-row",
                    column_config={
                        "청구 금액(USD)": st.column_config.NumberColumn("청구 금액(USD)", format="$%.2f"),
                        "연체 확률(%)": st.column_config.NumberColumn("연체 확률(%)", format="%.1f%%"),
                    },
                )

                valid_nos = set(result_frame["No"].astype(int).tolist())
                selected_no = selected_upload_no(chart_state, table_state, table_frame, default_no)
                if selected_no not in valid_nos:
                    selected_no = default_no
                    st.session_state["upload_selected_no"] = selected_no
                selected_row = result_frame[result_frame["No"].astype(int) == selected_no].iloc[0]
                selected_base_probabilities = base_probabilities_from_result(selected_row)

                with detail_container:
                    detail_col, judge_col = st.columns([1.05, 0.95], gap="large")
                    with detail_col:
                        st.markdown('<div class="probability-panel"><div class="panel-head">선택 인보이스 연체 확률</div></div>', unsafe_allow_html=True)
                        st.plotly_chart(
                            gauge_chart(float(selected_row["연체 확률(%)"]) / 100, artifact.threshold),
                            width="stretch",
                            config={"displayModeBar": False},
                        )
                        st.markdown(
                            f"""
                            <div class="panel-body-note">
                                선택 고객: <strong>{escape(str(selected_row["고객명"]))}</strong><br>
                                연체 확률: <strong>{float(selected_row["연체 확률(%)"]):.1f}%</strong><br>
                                업무 판단: <strong>{escape(str(selected_row["위험 등급"]))}</strong>
                            </div>
                            """,
                            unsafe_allow_html=True,
                        )
                    ...
                    with judge_col:
                        st.markdown('<div class="model-judge-panel"><div class="panel-head">모델별 판단</div></div>', unsafe_allow_html=True)
                        st.plotly_chart(
                            base_probability_chart(selected_base_probabilities),
                            width="stretch",
                            config={"displayModeBar": False},
                        )
                        st.markdown(
                            ...
                            <div class="note-panel">
                                <div class="notice">
                                    1. 선택한 인보이스 기준의 모델별 판단입니다.<br>
                                    2. 표의 행 또는 위 그래프의 바를 클릭하면 상세 그래프가 갱신됩니다.<br>
                                    3. 운영 기준선은 {artifact.threshold * 100:.1f}%입니다.<br>
                                    4. 평가 기준은 지급일 이후 5영업일 초과 연체입니다.
                                </div>
                            </div>
